CREATE DATABASE  IF NOT EXISTS `modeloott` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `modeloott`;
-- MySQL dump 10.13  Distrib 5.7.11, for Win64 (x86_64)
--
-- Host: 127.0.0.1    Database: modeloott
-- ------------------------------------------------------
...
--
-- Dumping routines for database 'modeloott'
--
/*!50003 DROP PROCEDURE IF EXISTS `filtrado` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `filtrado`(IN AnombreOT boolean, IN AdescripcionOT boolean, IN AnombreClu boolean,
	IN AdescripcionClu boolean, IN AnombreOf boolean, IN AnombreIn boolean, IN Aestado BOOLEAN, IN AnombreInv BOOLEAN,
	IN BnombreOT VARCHAR(45),IN BdescripcionOT VARCHAR(100), IN BnombreClu VARCHAR(45), IN BdescripcionClu VARCHAR(100),
    IN BnombreOf VARCHAR(45), IN BnombreIn VARCHAR(45), IN Bestado VARCHAR(45),IN BnombreInv VARCHAR(45))
BEGIN
create table tempTable (tnombreOT VARCHAR(150), tdescripcionOT VARCHAR(500), tnombreClu VARCHAR(45),
	tdescripcionClu VARCHAR(500), tnombreOf VARCHAR(45), tnombreIn VARCHAR(45), testado VARCHAR(45),tnombreInv VARCHAR(45), tidOT int);
create table tempTable1 (tnombreOT1 VARCHAR(150), tdescripcionOT1 VARCHAR(500), tnombreClu1 VARCHAR(45),
	tdescripcionClu1 VARCHAR(500), tnombreOf1 VARCHAR(45), tnombreIn1 VARCHAR(45), testado1 VARCHAR(45),tnombreInv1 VARCHAR(45), iidOT1 int);
insert into tempTable (tnombreOT,tdescripcionOT,tnombreClu, tdescripcionClu, tnombreOf, tnombreIn, testado,tnombreInv,tidOT)
select t.nombre,t.descripcion,c.nombre, c.descripcion, o.nombre, i.nombre,d.estado, r.nombre, t.idOT
	from ofertas_Tecnologicas t
    join cluster c on fkIdCluster = idCluster
    join oficina o on fkOficina = idOficina
    join direccion d on fkIdDireccion = iddireccion
    join institucion i on fkIdInstitucion = idInstitucion
    join ofertas_tecnologicas_has_investigadores on fkIdOT = idOT
    join investigadores r on fkIdInvestigadores= idInvestigadores;
select * from temptable;
SET SQL_SAFE_UPDATES = 0;
IF BnombreOT!="" THEN
	insert into tempTable1 (tnombreOT1,tdescripcionOT1,tnombreClu1, tdescripcionClu1, tnombreOf1, tnombreIn1, testado1,tnombreInv1,tidOT1)
		select * from temptable where tnombreOT=BnombreOT;
	delete from temptable;
	insert into tempTable (tnombreOT,tdescripcionOT,tnombreClu, tdescripcionClu, tnombreOf, tnombreIn, testado,tnombreInv,tidOT)
		select * from temptable1;
	delete from temptable1;
END if;
IF BdescripcionOT!="" THEN
	insert into tempTable1 (tnombreOT1,tdescripcionOT1,tnombreClu1, tdescripcionClu1, tnombreOf1, tnombreIn1, testado1,tnombreInv1,tidOT1)
		select * from temptable where tdescripcionOT=BdescripcionOT;
	delete from temptable;
	insert into tempTable (tnombreOT,tdescripcionOT,tnombreClu, tdescripcionClu, tnombreOf, tnombreIn, testado,tnombreInv,tidOT)
		select * from temptable1;
	delete from temptable1;
...
IF BnombreClu!="" THEN
	insert into tempTable1 (tnombreOT1,tdescripcionOT1,tnombreClu1, tdescripcionClu1, tnombreOf1, tnombreIn1, testado1,tnombreInv1,tidOT1)
		select * from temptable where tnombreClu=BnombreClu;
	delete from temptable;
	insert into tempTable (tnombreOT,tdescripcionOT,tnombreClu, tdescripcionClu, tnombreOf, tnombreIn, testado,tnombreInv,tidOT)
		select * from temptable1;
	delete from temptable1;
END if;
IF BdescripcionClu!="" THEN
	insert into tempTable1 (tnombreOT1,tdescripcionOT1,tnombreClu1, tdescripcionClu1, tnombreOf1, tnombreIn1, testado1,tnombreInv1,tidOT1)
		select * from temptable where tdescripcionClu=BdescripcionClu;
	delete from temptable;
	insert into tempTable (tnombreOT,tdescripcionOT,tnombreClu, tdescripcionClu, tnombreOf, tnombreIn, testado,tnombreInv,tidOT)
		select * from temptable1;
	delete from temptable1;
...
IF BnombreOf!="" THEN
	insert into tempTable1 (tnombreOT1,tdescripcionOT1,tnombreClu1, tdescripcionClu1, tnombreOf1, tnombreIn1, testado1,tnombreInv1,tidOT1)
		select * from temptable where tnombreOf=BnombreOf;
	delete from temptable;
	insert into tempTable (tnombreOT,tdescripcionOT,tnombreClu, tdescripcionClu, tnombreOf, tnombreIn, testado,tnombreInv,tidOT)
		select * from temptable1;
	delete from temptable1;
...
IF BnombreIn!="" THEN
	insert into tempTable1 (tnombreOT1,tdescripcionOT1,tnombreClu1, tdescripcionClu1, tnombreOf1, tnombreIn1, testado1,tnombreInv1,tidOT1)
		select * from temptable where tnombreIn=BnombreIn;
	delete from temptable;
	insert into tempTable (tnombreOT,tdescripcionOT,tnombreClu, tdescripcionClu, tnombreOf, tnombreIn, testado,tnombreInv,tidOT)
		select * from temptable1;
	delete from temptable1;
...
IF Bestado!="" THEN
	insert into tempTable1 (tnombreOT1,tdescripcionOT1,tnombreClu1, tdescripcionClu1, tnombreOf1, tnombreIn1, testado1,tnombreInv1,tidOT1)
		select * from temptable where testado=Bestado;
	delete from temptable;
	insert into tempTable (tnombreOT,tdescripcionOT,tnombreClu, tdescripcionClu, tnombreOf, tnombreIn, testado,tnombreInv,tidOT)
		select * from temptable1;
	delete from temptable1;
...
IF BnombreInv!="" THEN
	insert into tempTable1 (tnombreOT1,tdescripcionOT1,tnombreClu1, tdescripcionClu1, tnombreOf1, tnombreIn1, testado1,tnombreInv1,tidOT1)
		select * from temptable where tnombreInv=BnombreInv;
	delete from temptable;
	insert into tempTable (tnombreOT,tdescripcionOT,tnombreClu, tdescripcionClu, tnombreOf, tnombreIn, testado,tnombreInv,tidOT)
		select * from temptable1;
	delete from temptable1;
END if;
IF AnombreOT=false THEN
	alter table temptable drop column tnombreOT;
END if;
IF AdescripcionOT=false THEN
	alter table temptable drop column tdescripcionOT;
END if;
IF AnombreClu=false THEN
	alter table temptable drop column tnombreClu;
END if;
IF AdescripcionClu=false THEN
	alter table temptable drop column tdescripcionClu;
END if;
IF AnombreOf=false THEN
	alter table temptable drop column tnombreOf;
END if;
IF AnombreIn=false THEN
	alter table temptable drop column tnombreIn;
END if;
IF Aestado=false THEN
	alter table temptable drop column testado;
END if;
IF Aestado=false THEN
	alter table temptable drop column tnombreInv;
END if;
select *from temptable;
...
/*!50003 DROP PROCEDURE IF EXISTS `inputUsuario` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_ALL_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `inputUsuario`(IN usuar VARCHAR(45), IN contra VARCHAR(45),
 OUT tipoS VARCHAR(45), OUT encont boolean)
BEGIN
DECLARE found INTEGER DEFAULT 0;
DEClARE usuarCursor CURSOR FOR
SELECT tipo FROM usuario WHERE usuario = usuar AND contrasena=contra;
DECLARE CONTINUE HANDLER FOR NOT FOUND SET found = -1;
open usuarCursor;
fetch usuarCursor into tipoS;
if found != -1 then
	set encont=true;
else 
	set encont=false;
...
-- Dump completed on 2016-09-08 18:07:16
